...
지금까지 구현의 문제점은 높이가 넘지 않더라도 아래와 같이 무조건 더보기/접기 버튼이 보인다는 겁니다.
...
<ArticleImage
  img={i5}
  alt="긴 글일 때만 더보기 버튼이 보이고 짧은 글은 더보기 버튼이 안보인다!"
  caption="긴 글일 때만 더보기 버튼이 보이고 짧은 글은 더보기 버튼이 안보인다!"
  border
/>
...
- **애니메이션**: 높이 값이 auto가 아니라 명시적으로 관리하고 있으므로 애니메이션을 주기가 좋습니다. [React Spring](https://www.react-spring.dev/) 같은 걸 잘 이용하면 어떻게 되지 않을까 싶습니다. ㅎㅎ
- **꼼꼼한 리렌더링**: 위 코드에는 `ref`가 초기화되는 타이밍에 따라 더보기 버튼이 보이지 않을 수 있는 문제가 있을 수 있습니다. `ref`가 초기화되는 타이밍을 좀 더 자세히 공부해오고 확실히 해보겠습니다. ㅠㅠ

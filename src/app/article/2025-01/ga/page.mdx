...
GTM은 태그, 트리거, 변수를 아주 다양하게 즉시 사용할 수 있어서 개발 공수가 거의 없다는 점도 큰 장점입니다. 예를 들어 [요소 공개 상태](https://support.google.com/tagmanager/answer/7679410?hl=en)라는 트리거가 있는데, 사실 이는 [Intersection Observer](https://developer.mozilla.org/en-US/docs/Web/API/Intersection_Observer_API)의 동작을 모방한 것에 지나지 않지만 코드를 건드리지 않고 UI로 뚝딱 설정할 수 있으니 완전 편리하죠. 변수를 계속해서 확장해나갈 수 있고 정규 표현식이나 CSS Selector도 사용할 수 있어서 활용성이 높습니다.
...
- **이벤트(Events)**: 웹사이트에 방문한 사용자가 일으키는 행위입니다. 간단하게는 페이지뷰, 클릭, 스크롤 등이 있습니다. [자동으로 수집되는 이벤트](https://support.google.com/analytics/answer/9234069?hl=en)도 있고 우리가 직접 이벤트를 만들어 보내줄 수도 있습니다. 너네들이 형식에 맞게 보내주면 우리가 좀 보기 좋게 분석해줄게~ 하는 [추천 이벤트](https://developers.google.com/analytics/devguides/collection/ga4/reference/events?hl=en&client_type=gtm)도 있습니다. GA에서 분석할 때 가장 기본이 되는 단위입니다.
- **사용자(Users)**: 웹사이트에서 활동하는 사람입니다.
- **매개변수(Parameters)**: 이벤트에 붙는 부가 정보입니다. 마찬가지로 사용자가 임의로 추가할 수도 있고 자동으로 수집되는 매개변수도 있습니다. 이벤트가 수집하는 시점에서 자주 사용되는 용어입니다. (분석할 때는 후술할 측정기준 혹은 측정항목으로 구분됩니다)
...
만약 탐색 보고서를 만드는데 이렇게 해놓으면 측정 기준으로는 "인스타그램"밖에 나오지 않습니다. 왜냐하면 이미 세그먼트가 그렇게 좁혀져있기 때문입니다. 첫 사용자 매체에 따라 분류를 하고 싶다면 세그먼트를 "모든 사용자"로 설정하거나 중첩되지 않는 다른 조건으로 설정해야 할 것입니다.

## GA 만들기

[GA 콘솔](https://analytics.google.com/analytics/web)에서 계정을 만드세요. 만들었다면 속성을 만듭시다.
...
<ArticleImage img={gtm2} alt="Google 태그 관리자 설치 안내" border caption="Google 태그 관리자 설치 안내" />
...
- **풍부한 정보 제공**: GA에서 데이터를 분석하기 위해서는 그 서비스에만 존재하는 다양한 정보를 팍팍 주는 게 좋습니다. 예를 들어 변호사를 대상으로 어떤 서비스를 전개하고 있어서 전문 영역(민사, 형사, 노동법 등등)을 사용자로부터 입력받고 있다면, 그를 사용자 정보로서 GA로 넘겨줄 수 있습니다. 그러면 분석할 때 세그먼트로 활용할 수 있어서 좋을 것입니다. 클릭 이벤트를 만들 때 *페이지 유형*이라는 정보를 추가하는 방법을 후술하겠습니다.
...
<ArticleImage img={t5} alt="실행된 태그의 자세한 내용" border caption="실행된 태그의 자세한 내용" />
...
<ArticleImage img={t6} alt="DebugView 에서 page_type 값이 넘어오고 있는 모습" border caption="DebugView 에서 page_type 값이 넘어오고 있는 모습" width={450} />

제대로 넘어오고 있습니다!

하지만 방심은 금물... DebugView는 정말 디버그 용도일 뿐이고 실제로 데이터가 쌓이려면 별도의 GA4 세팅이 필요합니다. 측정기준을 새로 만드려면 관리 - 맞춤 측정항목에서 `page_type`을 직접 추가해줍시다.

<ArticleImage img={t7} alt="GA4 새 맞춤 측정기준" border caption="GA4 새 맞춤 측정기준" />

실제 보고서에서 확인할 수 있는 데이터는 그 전날까지 쌓인 데이터이므로 하루 기다려서 데이터를 한번 살펴보세요.
...
2. **기능 추가 및 개선 + `data-gtm-id` 및 `page_type` 설정** : 개발자가 개발을 열심히 합니다. 그리고 추가/개선한 사항에 대해 GA에 데이터가 쌓일 수 있도록 설정해놓습니다.
4. **배포 및 문서 갱신**: 개발 내용이 운영에 반영됩니다. 마케터/기획자가 데이터를 확인할 수 있도록 개발자가 문서를 잘 최신화 해놓습니다.
5. **시간 경과**: 실제 고객이 추가/개선된 기능을  사용하면서 데이터가 수집됩니다.
6. **분석**: 마케터/기획자가 GA에서 `page_type` 및 클릭 이벤트 기반으로 데이터를 분석하고 성과를 평가합니다.

여기서의 포인트는 GTM에서 수집하는 내용이 자동화되어 있어서 기능이 변경될 때마다 GTM을 건드릴 필요가 없다는 점입니다. 변수를 활용하지 않는다면 매번 트리거와 태그를 추가/변경해줘야 하는 불편함이 있습니다.

이정도면 개발자로서 최고의 GA 환경을 깔아주는 게 아닌가 감히 생각해보면서 글을 마무리해보겠습니다. 모두들 즐거운 협업 되세요!
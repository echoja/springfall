...
import setting7 from "./setting7.png";
...
VSCode에서 Trigger Suggest라는 기능을 적극적으로 사용할 예정인데, 단축키가 `⌃스페이스`로 똑같기 때문에 입력 소스 단축키를 사용하지 않도록 합니다. Caps Lock 위치에 있는 한/영 버튼을 주로 사용하기도 하구요. 그 바로 아래 것도 어차피 쓰지 않으니 옵션을 껐습니다.
...
<ArticleImage
  border
  img={setting7}
  alt="손쉬운 사용 - 포인터 제어 - 트랙패드 옵션... - 드래그에 트랙패드 사용"
  caption="손쉬운 사용 - 포인터 제어 - 트랙패드 옵션... - 드래그에 트랙패드 사용"
/>

저는 마우스도 키보드도 안쓰고 맥북 하나만 쓰는데요, 그래서 트랙패드에서 드래그를 세 손가락으로 쉽게 하려고 설정했습니다.
...
이 설정은 일단 지금 단계에서 곧장 설정할 것은 아니지만 일단 "시스템 설정"에서 하는 거라 여기에 포함했습니다. 인터넷으로 [Google Meet](https://meet.google.com/)을 하거나 [Gather(게더타운)](https://ko.gather.town/)을 하거나 [Zoom](https://www.zoom.com/ko) 미팅을 할 때 **화면 공유**를 하게 되면 최초에 한해 저 설정을 해줘야 합니다. 그러나 바로 되는 것이 아니라 프로그램을 껐다 켜야 합니다. 회의 진행에 지장이 있을 수도 있으니 미리 준비해놓아요.
...
<ArticleImage border img={q1} alt="Fig → Amazon" caption="Fig → Amazon" />
...
- **Git 정보**: 현재 어떤 브랜치인지, pull 또는 push 할 내용이 있는지 등을 표시해줍니다.
...
[asdf](https://asdf-vm.com/guide/getting-started.html)란 Node.js와 같은 개발에 필요한 프로그램의 버전을 관리해주는 프로그램입니다. 프로젝트마다 버전을 명시하여 따로따로 버전을 사용할 수 있도록 해줍니다. 버전 관리라고 한다면 [nvm](https://github.com/nvm-sh/nvm)같은 프로그램이 떠오를텐데요, asdf이 그 역할을 대신한다고 보면 됩니다. asdf는 nodejs 뿐만 아니라 python등 다양한 버전 크리티컬한 환경을 제어하는 데 좋습니다.

만약 상황상 nvm을 꼭 사용해야 한다면 asdf로는 nodejs를 설치하지 마세요. 꼬일 확률이 높아집니다.
...
<ArticleImage
  border
  img={fzf}
  alt="fzf로 최근 명령어 history를 검색하는 모습"
  caption="fzf로 최근 명령어 history를 검색하는 모습"
  width={500}
/>
...
## direnv: 폴더 기반 환경변수 설정 (필수 아님)

[direnv](https://direnv.net/)는 폴더 기반으로 터미널 상에서 환경변수를 자동으로 설정하고 unset 해주는 녀석입니다. 대부분의 프로젝트는 환경변수를 알아서 적절하게 세팅하므로 필요 없는 경우가 많습니다.
...
그리고 원하는 프로젝트 파일에 `.env`와 `.envrc` 파일을 넣습니다.
...
이렇게 하고 폴더에 쉘을 띄우면 아래와 같은 에러가 뜰텐데요,

```sh
direnv: error /Users/th.kim/Desktop/test/.envrc is blocked. Run `direnv allow` to approve its content
```

시키는 대로 `direnv allow`를 해줍니다.
...
그러면 `TEST` 환경변수를 direnv가 로딩했다고 뜹니다.

...
우리가 `http://localhost:5173/` 라는 경로로 접근하게 되면 로컬에 떠있는 서버는 그 경로를 분석해 대략 다음과 같은 절차로 요청을 처리합니다.
...
해결법은 간단합니다. 저 preamble 코드를 직접 삽입하면 됩니다.
...
이제 더이상 `@vitejs/plugin-react can't detect preamble` 에러가 발생하지 않는다는 걸 확인할 수 있습니다. 
...
Webpack 에는 Module Federation 이라는 개념이 있습니다. 거대한 프론트엔드 프로젝트를 한번에 빌드하는 게 아니라 잘게 나누어서 필요한 부분만 빌드한다는 개념인데, 쉽지는 않습니다.

## 구현 ㄱㄱ


이제 준비는 끝났습니다. 파일의 변경사항이 있을 땐 다음과 같은 단계로 진행됩니다.

1. 서버는 로컬 파일들을 지켜봅니다(watch). 변경사항이 감지되면 WebSocket으로 브라우저에게 알립니다.
1. 브라우저는 WebSocket으로 정보를 받습니다.
1. 그 정보를 토대로 어떤 부분을 어떻게 바꿀지 잘 판단해서 `accept` 등의 함수를 적절하게 호출합니다.
1. `accpet` 함수는 `react-refresh` 함수를 호출합니다.

React 팀이 관리하는 `react-refresh`라는 패키지는 React 개발 환경에서 컴포넌트를 Fast Refresh할 수 있도록 도구를 제공합니다. Vite가 이 도구를 사용하는 거구요. 
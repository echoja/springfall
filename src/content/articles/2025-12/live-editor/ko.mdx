...
import i0 from "./assets/i0.png";
import i1 from "./assets/i1.jpg";
import i2 from "./assets/i2.png";
import i3 from "./assets/i3.png";
import i4 from "./assets/i4.png";
import i99 from "./assets/i99.png";
...
- [크리마(CREMA)](https://www.cre.ma/): 리뷰 데이터 기반 마케팅 플랫폼
...
Tailwind CSS 를 적용하려면 [공식 문서를 일단 따라하면](https://tailwindcss.com/docs/installation/using-vite) 됩니다.
...
다른 문제도 있습니다. [Shadow DOM 내부에서는 CSS `@property` 기능이 동작하지 않습니다. (tailwindcss#15005)](https://github.com/tailwindlabs/tailwindcss/issues/15005). Tailwind 문제는 아니고 표준이 정해지지 않은 문제라 브라우저에서 구현도 되지 않은 상태입니다. 저는 border 스타일만 정해지지 않아서 문제를 살펴봤었는데요, Tailwind v4는 `border-style`이 `@property`로서 기본값이 정해지는데, 이 기본값 조차 Shadow DOM 내부에서 유효하지 않았기 때문에 아예 안보였던 것입니다!
...
`live-locator`에는 React가 포함되어 있습니다. 그래서 React 개발에 필요한 정보도 함께 담겨져 있고, Node.js에서 `live-locator`가 또다시 빌드될 것을 전제로 하여 `process.env.NODE_ENV` 에 따른 분기 처리도 보존되었습니다. 그런데 `live-locator`는 브라우저에서 구동되어야 하고 엄밀히 라이브러리가 아니라 최종 결과물입니다. 그러므로 정보를 보존할 필요가 없습니다.
...
Production-Ready 라고 제목에 써넣어두긴 했지만 실제 서비스에 적용할 땐 해야 할 게 더 많긴 합니다. 특히 `live-locator`와 같은 기능이 로드되는 건 권한 체크(관리자 인증)가 필요하죠. 그외 에러/로그 수집과 같은 운영에 필요한 부분도 신경써야 할 것입니다.

다른 사이트에서 동작하는 앱을 만든다는 건, 정말 어렵습니다. 기존 도구들은 그런 상황을 전제해놓지 않았죠, 사이트는 건드리면 안되죠, 또한 사이트로부터 영향을 받아서도 안됩니다. 각종 제약은 많으면서도 개발 생산성도 좋아야 하죠. 이 글에서는 `main`과 `live-locator`를 분리하고, 조건부 로딩, Shadow DOM 격리, Vite HMR 보정 같은 현실적인 문제를 하나씩 해결해봤습니다.
...
- [Andar 공식 사이트](https://andar.co.kr/)
- [Beusable 공식 사이트](https://www.beusable.net/)
- [CREMA 공식 사이트](https://www.cre.ma/)
- [Google Tag Manager](https://tagmanager.google.com/)
- [Vercel Live Feedback 스크립트 (minified)](https://vercel.live/_next-live/feedback/feedback.js)
- [mini-multi-scripts GitHub 저장소](https://github.com/echoja/mini-multi-scripts)
- [mini-multi-scripts 라이브 데모](https://mini-multi-scripts-live-locator.vercel.app/)
...
- [데모 main.js (minified)](https://mini-multi-scripts-live-locator.vercel.app/main.js)
- [데모 live-locator.js (minified)](https://mini-multi-scripts-live-locator.vercel.app/live-locator/live-locator.js)

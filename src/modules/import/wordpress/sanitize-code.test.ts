...
test("simple", () => {
  const result = sanitizeCode("abc<code>#include <stdlib.h></code>abcd");
  expect(result).toEqual("abc<code>#include &lt;stdlib.h&gt;</code>abcd");
});

test("simple + newline", () => {
  const result =
    sanitizeCode(`<pre class="wp-block-code language-c"><code>/* 노미네뜨 검증 X */
  #include <stdlib.h>
  #include <stdio.h>
...
test("code 태그 안의 문자들이 모두 sanitize 되어야 함", () => {
  const result = sanitizeCode(fixture);
  expect(result).toMatchInlineSnapshot(`
    "<!-- wp:heading -->
    <h2>구조체(<code>struct</code>)</h2>
    <!-- /wp:heading -->
